...
const updateStatus = async (status) => {
  const res = await fetch(
    `http://localhost:5000/adoption/${data._id}`,
    {
      method: "PATCH",
      headers: {
...
<Button variant="outline" className='rounded-md' onClick={()=>handleRequest(userId)}>Request</Button>
      </Modal.Trigger>
      <Modal.Backdrop>
        <Modal.Container>
          <Modal.Dialog className="sm:max-w-[360px] relative">
          
            <Chip  className={`absolute top-3 right-3`} color={`${isClick=='Pending' ?'warning': isClick =='Approve'? 'success':'danger'}`}>{isClick}</Chip>
            <Modal.Header>
            ...
            <Modal.Footer>
              ...
              <Button onClick={handleApprove}  slot="close" className='rounded-md bg-linear-to-tr from-purple-500 to-blue-500' >Approve</Button>
               {/* {
                isClick =='pending' &&
                <>
                 <Button onClick={handleReject} slot="close" className='rounded-md' variant="secondary">
